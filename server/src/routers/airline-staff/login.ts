...
router.post('/staff/login', async (req: Request, res: Response, next: NextFunction) => {
    ...
    const existingUser = await databaseManager.singleRecordQueryTable({
        query: `SELECT * FROM ${Tables.AIRLINE_STAFF} WHERE email=?`,
        values: [email]
    });
...
export { router as airlineStaffLoginRouter }
